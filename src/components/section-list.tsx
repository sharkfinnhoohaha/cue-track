...
export function SectionList({ sections, onChange }: SectionListProps) {
  ...
  const addSection = useCallback((name: string = 'New Section', bars: number = 8) => {
    onChange([...sections, { id: generateId(), name, bars }]);
  }, [sections, onChange]);

  const updateSection = useCallback((index: number, update: Partial<SongSection>) => {
    onChange(sections.map((s, i) => (i === index ? { ...s, ...update } : s)));
  }, [sections, onChange]);
  ...
  const handleDragOver = useCallback((e: React.DragEvent<HTMLDivElement>, index: number) => {
    e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setOverIndex(index);
  }, []);
  ...
  const handleDragEnd = useCallback(() => {
    ...
    setDragIndex(null); setOverIndex(null); dragNodeRef.current = null;
  }, []);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="label font-mono text-xs uppercase tracking-wider">Sections</label>
        {sections.length > 0 && (
          <span className="font-mono text-xs text-muted">{sections.length} section{sections.length !== 1 ? 's' : ''} / {totalBars} bars</span>
        )}
      </div>
      <div className="space-y-2">
        {sections.map((section, index) => (
          ...
          >
            ...
            <button type="button" onClick={() => removeSection(index)} className="flex-shrink-0 rounded-md p-1.5 text-muted hover:bg-red-900/30 hover:text-red-400 transition-colors" aria-label={`Delete section ${section.name}`}>
      ...
      <Button type="button" variant="secondary" size="sm" onClick={() => addSection()} className="w-full">

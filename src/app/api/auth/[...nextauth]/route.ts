      ...
      async jwt({ token, user }) {
        ...
        if (process.env.DATABASE_URL && token.id) {
          try {
            ...
            const rows = await db.select({ subscriptionStatus: users.subscriptionStatus, stripeCustomerId: users.stripeCustomerId }).from(users).where(eq(users.id, token.id)).limit(1);
            if (rows.length > 0) {
              token.subscriptionStatus = rows[0].subscriptionStatus;
              token.stripeCustomerId = rows[0].stripeCustomerId;
            }
          } catch { /* continue with stale token */ }
        }
        return token;
      },
